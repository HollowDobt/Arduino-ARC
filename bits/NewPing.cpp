...
//等待回声的过程当中是阻塞的（无法执行其他的程序语句），但是中断可以正常进行

#include "NewPing.h"

//构造一个新的超声波传感器对象
NewPing::NewPing(uint8_t trigger_pin, uint8_t echo_pin, unsigned int max_cm_distance) {
//这是一个标志位，默认是false，然后会执行引脚的赋值操作，好像是位模式啥的，看下面的代码里有把数字转化为二进制代码的
//小tips，函数内的参数一般单词之间通过下划线分割开，并且不采用首字母大写
//小tips，全局变量cpp里喜欢用在最前面加下划线来表示，这样变量命名不容易混淆
#if DO_BITWISE == true
	...
	//自动检测这是不是一个单引脚的超声传感器，每次构造函数的时候会自动判断执行一次
...
unsigned int NewPing::ping(unsigned int max_cm_distance) {
	...
	if (!ping_trigger()) return NO_ECHO; // Trigger a ping, if it returns false, return NO_ECHO to the calling function.
		...
		while (*_echoInput & _echoBit)    // Wait for the ping echo.
	#else
		while (digitalRead(_echoPin))     // Wait for the ping echo.这里其实意味着，一个传感器在长时间没有收到回声后，也会自动把echo变为0
	#endif
			if (micros() > _max_time) return NO_ECHO; // Stop the loop and return NO_ECHO (false) if we're beyond the set maximum distance.
#endif

	return (micros() - (_max_time - _maxEchoTime) - PING_OVERHEAD); // 扣掉一个1毫秒的损耗感觉很好笑，基本上感受不到什么差异
}
//Ping方法二，回传整厘米数
...
//让超声波传感器触发，核心就是拉高引脚10微秒（是很短的时间），如果说trig的过程中上一次测量还没有结束则会trig失败，如果说等待了过长的时间echo没有被拉高，则也认为trig失败
//成功触发后，会在echo拉高的那一刻设置maxtime，并且返回trig的成功，然后开始等待回声的结束
...
//设置最大检测距离，最大检测距离是和最大回声时间关联在一起的
//rounding未开启的时候是直接按整厘米数对应的回声时间（以微秒为单位）
...
//和前面的函数不一样，这里是传入回声时间，然后用于计算厘米值
//里面的内容不复杂，首先是判断是否需要取整，但是很奇怪为什么是没有开启的时候进行的反而是取整操作？
//另外如果取整开启了，也不存在NewPingConvert这个函数，让我有点难以捉摸
...
//如果定时器启动了并且是按照位运算的方式工作的，那么会执行后面的一坨东西，但是这边并不会执行
